...
if not os.path.exists(INDEX_PATH):
    with open(INDEX_PATH, 'w', encoding='utf-8') as f:
        f.write("""<!DOCTYPE html><html><head><title>WETMO</title></head><body><h1>WETMO</h1></body></html>""")
    print(f"⚠️ Создан пустой {INDEX_PATH}")
...
def chat(target):
    ...
    if not t or u.username == t.username: 
...
def admin_v(uid):
    ...
    if u and u.username == 'wetmo':
        ...
        if t: 
            t.is_verified = not t.is_verified
            db.session.commit()
    ...
    if u and u.username == 'wetmo':
        ...
        if t and t.username not in ['wetmo', 'wetmo_auth']:
            Message.query.filter(
                (Message.sender == t.username) | (Message.chat_id.contains(t.username))
            ).delete()
            db.session.delete(t)
            db.session.commit()
    return redirect(url_for('admin'))
...
if __name__ == '__main__':
    ...
    print(f"📄 index.html существует: {os.path.exists(INDEX_PATH)}")
